...
cache_list *init_cache_list() {
	cache_list *list = Malloc(sizeof(*list));
	...
	if(list) {
		if(node) {
			while(list->bytes_left < node->length) {
				cache_node *tmp_node = evict_list(list);
...
cache_node *remove_node(char *index, cache_list *list){
	if(list) {
		cache_node *tmp = search_node(list, index);
		if(tmp) {
			if(tmp == list->head)
				tmp = evict_list(list);
			else {
...
int insert_content_node(cache_list *list, char *index, char *content, unsigned int len){
	...
	cache_node *tmp = Malloc(sizeof(*tmp));
	init_node(tmp);
